...
export async function GET(request: Request) {
  ...
  for (const item of categorySalesResult.data || []) {
    const productData = item.products as unknown as { category_id: string; categories: { name: string } | null } | null
  ...
  return NextResponse.json({
    ...
    topProducts: Object.values(productMap).sort((a, b) => b.sold - a.sold).slice(0, 10),
    ...
    summary: {
      ...
      averageOrder: (ordersResult.data || []).length > 0
        ? (ordersResult.data || []).reduce((s, o) => s + (o.total || 0), 0) / (ordersResult.data || []).length
        : 0,
    },
  })
}

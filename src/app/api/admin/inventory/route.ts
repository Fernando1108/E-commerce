...
export async function POST(request: Request) {
  ...
  if (!product_id || !type || !quantity) {
    return NextResponse.json({ error: 'product_id, type y quantity son obligatorios' }, { status: 400 })
  }
  ...
  // Insert movement
  const { data, error: dbError } = await supabase.from('inventory_movements').insert({
    product_id,
    type,
  ...
  const stockChange = type === 'out' ? -Math.abs(quantity) : type === 'in' ? Math.abs(quantity) : quantity
  try {
    await supabase.rpc('update_product_stock', { p_product_id: product_id, p_quantity_change: stockChange })
  } catch {
    ...
    if (product) {
      await supabase.from('products').update({ stock: (product.stock || 0) + stockChange }).eq('id', product_id)
    }
  }

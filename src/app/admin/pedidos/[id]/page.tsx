...
interface OrderDetail {
  ...
  items: { id: string; product_id: string; quantity: number; price: number; products?: { name: string; image_url: string | null; price: number } }[];
}
  ...
  return (
    <div className="max-w-4xl space-y-6">
      <div className="flex items-center gap-4">
        ...
        <span className={`inline-flex items-center px-3 py-1 rounded-lg text-xs font-bold ${statusColors[order.status] || 'bg-slate-100 text-slate-600'}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Order items */}
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="lg:col-span-2 bg-white rounded-xl border border-slate-200 overflow-hidden">
              ...
              <div key={item.id} className="flex items-center gap-4 px-5 py-3">
                <div className="size-12 rounded-lg bg-slate-100 overflow-hidden flex-shrink-0">
                  {item.products?.image_url ? (
                    ...
                    <div className="size-full flex items-center justify-center text-slate-300"><Icon name="PhotoIcon" size={16} /></div>
                  )}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-slate-800 truncate">{item.products?.name || 'Producto'}</p>
        ...
        <div className="space-y-4">
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="bg-white rounded-xl border border-slate-200 p-5 space-y-3">
          ...
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="bg-white rounded-xl border border-slate-200 p-5 space-y-3">

...
/* ─── Inline UI primitives (previously in features/auth) ──────── */
type AuthFieldProps = {
  label: string;
  type: string;
  placeholder: string;
...
type StatusMsg = { status: 'idle' | 'loading' | 'success' | 'error'; message: string | null };

function StatusMessage({ status, message }: StatusMsg) {
...
function LoginContent() {
  ...
  const onSubmit = handleSubmit(async ({ email, password }) => {
    ...
    if (error) {
      setStatus('error');
      setFeedbackMessage(error.message === 'Invalid login credentials'
          ...
          <div className="grid gap-10 lg:grid-cols-[1.05fr_0.95fr] lg:gap-16 items-start">
            ...
            <div className="max-w-2xl pt-4 lg:pt-10">
              ...
              <p className="mt-6 max-w-xl text-base lg:text-lg leading-relaxed text-[#5A5A5A]">
                Accede con tu cuenta para retomar pedidos, revisar tu carrito y mantener
                una experiencia consistente en todo el storefront.
              </p>
